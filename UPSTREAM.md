...
- Packaging deviations (this fork only, no rule changes):
  ...
  - Bundled to committed `dist/` as ESM `.mjs` (esbuild, `--packages=external`) — Node refuses type-stripping `.ts` under `node_modules`, and a CJS bundle double-wraps the default export in a way Oxlint's plugin loader cannot unwrap; ESM preserves the original module shape. Install-time `prepare` builds were also rejected because they re-resolve dependencies through the installer's registry. `src/` ships for reference/debugging; regenerate `dist/` with `pnpm build` after rule changes.
